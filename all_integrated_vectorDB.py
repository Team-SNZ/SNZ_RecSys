...
PATH = "/Users/nayoung/SiNear/User_100.csv"
df = pd.read_csv(PATH)
...
"""
docs = [
    Document(page_content=row["Profile"], metadata={"id": row["ID"]})
...
def recommender_node(state: MyState, top_k=3) -> Command[Literal["supervisor"]]:
    ...
    user_profile = state["profile"] # 현재 유저 profile

    # 유사한 사용자 검색
    similar_docs = vector_store.similarity_search(user_profile, k=top_k+1)
    rec_ids = []

    for doc in similar_docs:
        if doc.metadata["id"] != user_id:
            rec_ids.append(doc.metadata["id"])
